...
app.controller('ScheduleController',
  function($scope, $ionicPopup, $timeout, $ionicHistory, $rootScope, $ionicPlatform, BinusMaya, $q) {
      ...
      $(data).find('tr').each(function(i, a) {
        ...
        if (i > 0) {
          hasil.push({
            ...
            time: ht.eq(1).html(),
            state: ht.eq(2).html(),
            course: ht.eq(3).html().split("-")[1],
            code: ht.eq(3).html().split("-")[0],
    ...
    var refreshJadwal = function() {
      return $q(function(resolve, reject) {
        BinusMaya.checkLogin()
          ...
          .then(function(d) {
            console.log(d);
            var _schedule = $(d.result).find("table"),
            ...
            resolve({jadwal: JSON.parse(localStorage.jadwal), lastUpdate: localStorage.lastUpdate});
          }, function() {
